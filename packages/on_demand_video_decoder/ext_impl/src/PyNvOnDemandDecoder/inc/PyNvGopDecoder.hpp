...
class PyNvGopDecoder {
   ...
   public:
     ...
     * component for loading GOP binary files that can be used by higher-level
     * functions like LoadGopsToList (Python binding).
     * 
     * Binary File Format:
     * - Files must contain SerializedPacketBundle data (same format as one GetGOPList output element)
   ...
   */
   ...
   private:
     ...
     * needed to create SerializedPacketBundles. It is used by get_gop_list()
