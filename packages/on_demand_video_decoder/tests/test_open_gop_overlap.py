...
class TestAllDecodingApisOnOverlapFrames:
    ...
    def test_getgoplist_decode_from_gop_list(self, decoder, fid):
        """``GetGOPList`` + ``DecodeFromGOPList`` — per-file-bundle YUV path."""
        gop_data, _, _ = _get_gop(decoder, fid)
        frames = decoder.DecodeFromGOPList([gop_data], [OPEN_GOP_SAMPLE], [fid])
        self._assert_yuv_nv12(frames)

    @pytest.mark.parametrize("fid", OVERLAP_FIDS)
    def test_getgoplist_decode_from_gop_rgb(self, decoder, fid):
        """``GetGOPList`` + ``DecodeFromGOPRGB`` (deprecated) — single-bundle RGB path."""
        gop_data, _, _ = _get_gop(decoder, fid)
