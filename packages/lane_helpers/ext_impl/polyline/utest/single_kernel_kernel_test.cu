...
TEST(PolylineSamplingFullySharedKernelTest, SimpleRectanglePolyline) {
    // Simple axis-aligned rectangle with perimeter length 6.
    ...
    const float distances_to_sample[NUM_DISTANCES] = {0.0f, 0.5f, 1.0f, 2.0f, 3.0f, 3.5f,
    ...
    ASSERT_EQ(cudaSuccess, cudaMallocManaged(&distances_to_sample_gpu, NUM_DISTANCES * sizeof(float)));

    float* res_points = nullptr;
    ASSERT_EQ(cudaSuccess,
              cudaMallocManaged(&res_points, NUM_SAMPLES * NUM_DISTANCES * NUM_DIMS * sizeof(float)));

    // Initialize input polyline (single sample)
    for (int i = 0; i < NUM_POINTS; ++i) {
        for (int d = 0; d < NUM_DIMS; ++d) {
            points[i * NUM_DIMS + d] = rectangle[i][d];
        }
    }

    // Initialize distances to sample (shared across samples)
    for (int i = 0; i < NUM_DISTANCES; ++i) {
        distances_to_sample_gpu[i] = distances_to_sample[i];
    }

    // Launch kernel
    const dim3 block_dim(64, 1);
    ...
    for (int i = 0; i < NUM_DISTANCES; ++i) {
        for (int d = 0; d < NUM_DIMS; ++d) {
            EXPECT_NEAR(res_points[i * NUM_DIMS + d], expected[i][d], eps);
        }
    }
